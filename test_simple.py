...
def quick_test():
    ...
    response_en = assistant.chat(
        "Explain quantum computing in simple terms.", "English"
    )
    print(f"   English response received: {len(response_en)} chars")

    # Test 2: Spanish
    print("\n2. Testing Spanish response...")
    response_es = assistant.chat("¿Qué es la computación cuántica?", "Spanish")
    print(f"   Spanish response received: {len(response_es)} chars")

    # Test 3: French
    print("\n3. Testing French response...")
    response_fr = assistant.chat("Parle-moi de l'apprentissage automatique.", "French")
